...
(function() {
  ...
  var MediaType = function(mediaType) {
    ...
    function encoderProtobuf(messageInstance){
      //TODO: validate that arg should be a protobuf messageInstance only 
      var protobuf = _.compose(encodeWrappedMessage(messageInstance, WrappedMessage), encodeMessageInstance)(messageInstance);
      return codec.encodeBytesWithLength(protobuf);
    }

    function decoderProtobuf(){
      return f.lift(_.compose(decodeProtobufMessage,decodeWrappedMessage),_.identity)();
    }

    function encodeMessageInstance(messageInstance){
      return messageInstance.$type.encode(messageInstance).finish();
    }

    function encodeWrappedMessage(messageInstance,WrappedMessage){
      return function (encodedMedia) {
        var containerTypeName= messageInstance.$type.fullName;

        var containerTypeId = ProtostreamType.lookupProtostreamTypeByName(containerTypeName);

        var wrappedMessage={
          wrappedTypeId:containerTypeId.protostreamDescriptorId,
          wrappedMessage:encodedMedia
        };

        return WrappedMessage.encode(wrappedMessage).finish();
      }
    }

    function decodeProtobufMessage(wrappedMessage){
        var wrappedTypeId = wrappedMessage.wrappedTypeId;
        var wrappedMessage= wrappedMessage.wrappedMessage;
        var protostreamType = ProtostreamType.lookupProtostreamTypeById(wrappedTypeId);
        var root = ProtobufRoot.findRootByTypeName(protostreamType.protostreamTypeName);
        var userProtobufType= root.lookupType(protostreamType.protostreamTypeName);
        return userProtobufType.decode(wrappedMessage);
    }

    function decodeWrappedMessage(bytebuf){
      var length = DECODE_VINT(bytebuf);
      var wrappedMessage = WrappedMessage.decode(bytebuf.buf.slice(bytebuf.offset,bytebuf.offset+length));
  ...
  var WrappedMessage = (function() {
    ...
    return {
      encode:function(message){
        return wrappedMessage.encode(message);
      },
      decode:function(buf){
        return wrappedMessage.decode(buf);
      }
    }
  }());
  ...
  var ProtobufRoot = (function() {
    ...
    return {
      registerProtostreamRoot: function(root){
        protobufRoots.push(root);
        return protobufRoots;
      },
      findRootByTypeName: function(typeName){
        if(protobufRoots.length==0) return null;
        if(protobufRoots.length==1) return protobufRoots[0];
        return _.find(protobufRoots,function(protobufRoot){
          return f.existy(protobufRoot.lookup(typeName));
        });
      }
    }
  }());
