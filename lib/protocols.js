...
(function() {
  ...
  function fulfill(promise, value, header, logger) {
    logger.tracef('Complete promise for request(msgId=%d) with %s', header.msgId, u.str(value));
  ...
  var DecodeMixin = (function() {
    ...
    var DECODE_HEADER = f.actions(
        ...
        codec.decodeUByte()     // topology change marker
          ...
          return {
            ...
            status: values[3], topoMarker: values[4]
          }
        });
    ...
    function decodePrev(promise, header, bytebuf) {
      ...
      if (f.existy(prev))
        return fulfill(promise, _.isEmpty(prev) ? undefined : prev, header, logger);
      else
        return false;
    }
    ...
    function decodeObject(promise, header, bytebuf, action) {
      if (isSuccess(header.status)) {
        ...
        if (f.existy(obj)) return fulfill(promise, obj, header, logger);
        else return false;
      }

      return fulfill(promise, undefined, header, logger);
    }

    function decodeIfMore(decoder, promise, header, bytebuf) {
      var objs = [];
      do {
        var more = DECODE_UBYTE(bytebuf);
        if (more == 1) {
          var obj = decoder(bytebuf);
          if (f.existy(obj)) objs.push(obj);
          else return false;
        }
      } while (more == 1);

      return fulfill(promise, objs, header, logger);
    }

    function decodeError(promise, header, bytebuf) {
    ...
    function decodePromise(promise, header, bytebuf) {
      logger.tracef('Call decode for request(msgId=%d)', header.msgId);
      return promise.decoder(promise, header, bytebuf);
    }

    function fulfillPromise(promise, header, value) {
      logger.tracef('Complete promise for request(msgId=%d) with undefined', header.msgId);
      promise.success(value);
      return true;
    }
    ...
    return {
      decodeHeader: function(bytebuf) {
        try {
          ...
          logger.tracef("Read header(msgId=%d): opCode=%s, status=%s, topoMarker=%d",
                        header.msgId, header.opCode, header.status, header.topoMarker);
          return header;
        } catch(ex) {
      ...
      decodeBody: function(promise, header, bytebuf) {
        if (hasError(header))
          return decodeError(promise, header, bytebuf);
        else if (f.existy(promise.decoder))
          return decodePromise(promise, header, bytebuf);

        return fulfillPromise(promise, header, undefined);
      },
      ...
      decodeWithMeta: function(promise, header, bytebuf) {
        if (isSuccess(header.status)) {
          ...
          return f.existy(versioned)
              ? fulfill(promise, f.merge(versioned, lifespan, idle), header, logger)
              : false;
        }
        ...
        return function(promise, header, bytebuf) {
          if (shouldReturnPrev)
            return cond(header)
                ...
                : fulfill(promise, undefined, header, logger);
      ...
      decodeCountValues: function(promise, header, bytebuf) {
        ...
        return fulfill(promise, pairs, header, logger);
      },
      decodeStringPairs: function(promise, header, bytebuf) {
        var count = DECODE_VINT(bytebuf);
        var pairs = [], i = 0;
        while (i++ < count) {
        ...
        return fulfill(promise, obj, header, logger);
      },
      decodeValues: function(promise, header, bytebuf) {
        return decodeIfMore(DECODE_PAIR, promise, header, bytebuf);
      },
      decodeKeys: function(promise, header, bytebuf) {
        return decodeIfMore(DECODE_OBJECT, promise, header, bytebuf);
      },
      decodeVInt: function(promise, header, bytebuf) {
        var num = DECODE_VINT(bytebuf);
        return fulfill(promise, num, header, logger);
      },
      ...
      complete: function(f) {
        return function(promise, header) {
          return fulfill(promise, f(header), header, logger);
        }
      }
    }
  }());
  ...
  var ListenersMixin = (function() {
    ...
    function dispatchEvent(event, listenerId, bytebuf, emitFunc) {
      return function() {
        var emitter = emitters.get(listenerId);
        if (f.existy(emitter))
          return emitFunc(event, emitter, bytebuf, listenerId);
    ...
    return {
      addListener: function(event, listener, listenerId) {
        ...
        if (!f.existy(emitter)) {
          ...
          emitters.put(listenerId, emitter);
        }
  ...
  var IteratorMixin = (function() { // protocol 2.3+
    ...
    return {
      ...
      decodeIterId: function(promise, header, bytebuf) {
        var iterId = DECODE_STRING(bytebuf);
        return fulfill(promise, iterId, header, logger);
      },
      decodeNextEntries: function(promise, header, bytebuf) {
        ...
        if (count > 0) {
          ...
          return fulfill(promise, entries, header, logger);
        }

        return fulfill(promise, [], header, logger);
      }
    }
  }());
