...
(function() {
  ...
  var DecodeMixin = (function() {
    ...
    var DECODE_VERSIONED = f.actions([codec.decodeBytes(8), codec.decodeObject()],
    ...
    function decodeIfMore(decoder, promise, header, bytebuf) {
      ...
      return fulfill(promise, objs, header);
    }

    return {
      op: function(header) { return header[2]; },
      ...
      decodeWithMeta: function(promise, header, bytebuf) {
        if (isSuccess(status(header))) {
          ...
          return f.existy(versioned)
              ? fulfill(promise, _.extend(versioned, lifespan, idle), header)
              : false;
        }
      ...
      decodeVInt: function(promise, header, bytebuf) {
        ...
        return fulfill(promise, num, header);
      },
  ...
  var ListenersMixin = (function() {
    ...
    var emitters = utils.keyValueMap();
    var logger = utils.logger('listener');
    ...
    var DECODE_KEY_VERSION = f.actions([codec.decodeObject(), codec.decodeBytes(8)],
    ...
    function emitKeyVersion(event, emitter, bytebuf, listenerId) {
      ...
      logger.tracel(function() {
        return ['Emit %s event for key=%s and version=%s',
                event, keyVersion.key, keyVersion.version.toString('hex')]; });
      emitter.emit(event, keyVersion.key, keyVersion.version, listenerId);
      return true;
    }
    ...
    function hasOptIncludeState(opts) {
      return _.has(opts, 'includeState') && f.thuthy(opts['includeState']);
    }
    ...
    return {
      ...
      encodeListenerAdd: function(listenerId, opts) {
        var includeState = hasOptIncludeState(opts) ? 1 : 0;
        return function() {
      ...
      decodeEvent: function(header, bytebuf) {
        ...
        if (f.existy(listenerId)) {
          ...
          return dispatcher(this.op(header));
        }
        return true;
      }
    }
