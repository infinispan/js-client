...
(function() {
  ...
  function fulfill(promise, value, header, logger) {
    logger.tracef('Complete promise for request(msgId=%d) with %s', msgId(header), u.str(value));
      ...
      decodeWithMeta: function(promise, header, bytebuf) {
        if (isSuccess(status(header))) {
          ...
          return f.existy(versioned)
              ? fulfill(promise, _.extend(versioned, lifespan, idle), header, logger)
              : false;
        }
        ...
        if (count > 0) {
          ...
          while (i++ < count) {
            ...
            if (hasMeta) {
              ...
              _.extend(entry, lifespan, idle, version);
            }
