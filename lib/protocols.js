...
(function() {
  ...
  var SASLMixin = (function() {
    ...
    return {
      ...
      sasl: function(authOpts) {
        console.log(authOpts);
        return function () {
            switch (authOpts.saslMechanism) {
            case 'PLAIN':
              logger.debugf('Try auth with PLAIN');
              logger.debugf('User ' + authOpts.userName);
              logger.debugf('Password ' + authOpts.password);
              var response = '';
              response += '\0';
            ...
            default:
              throw new Error('Unsupported auth mechanism ' + authOpts.saslMechanism);
          }
        }
      },
      decodeSasl: function(header, buff) {
        console.log('call decode');
        return {result: undefined, continue: true};
      }
    }
  }());
