(function() {
  ...
  exports.encodeProtobuf= function(obj,root){
    return root.encode(obj).finish();
  }

  exports.encodeWrappedMessage = function(message,typeId){
    var wrappedMessage={};
    if(bufferCheck(message)){
      wrappedMessage.wrappedMessage=message;
      wrappedMessage.wrappedTypeId=typeId;
    }else{
      //it can be extended for the basic data types;
    }
    return WrappedMessage.encode(wrappedMessage).finish();
  }

  // function doEncodeQueryObj(queryObj){
  //   
  // }
  ...
  var bufferCheck = f.validator('must be a buffer',isBuffer);
