(function() {
  ...
  function encodeProtobuf(message,typeId){
    ...
    var messageTypeId = typeId.apply(null,[message.$type.fullName]);
    return _.compose(encodeProtobufInstance(WrappedMessage), createWrappedMessage)(encodedMessage,messageTypeId);
  }
  ...
  function doDecodeProtobuf(bytebuf,protostreamTypename,root){
    ...
    if(f.existy(wrappedObject.wrappedMessage)){
  ...
  function trimBytebuf(bytebuf,length){
    ...
    return bytebuf.buf.slice(bytebuf.offset,bytebuf.offset+length);
  }
