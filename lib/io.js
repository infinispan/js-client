...
(function() {
  ...
  var Connection = function(addr, transport, listeners) {
      ...
      while (!replayable.isEmpty() && canDecodeMore) {
        ...
        if (canDecodeMore) {
          ...
          var topology = header.hasNewTopology
              ? transport.updateTopology(bytebuf, addr)
              : undefined;
    ...
    function ping(transport, conn) {
      var ctx = transport.context(16);
    ...
    function auth(transport, conn) {
      ...
      return transport.writeCommandPinned(ctx, p.decodeSasl, conn);
    }
    ...
    var conn = {
      ...
      auth: function () {
        ...
        conn.authDone = true;
        return auth(transport, conn).then(function () {
            conn.authDone = true;
        });
      },
      disconnect: function() {
  ...
  var FixedRouter = function(logger, conn, clusterName) {
    ...
    return {
      ...
      findConnection: function(triedAddrs) {
        return _.contains(triedAddrs, conn.getAddress()) ? undefined : Promise.resolve(conn);
      },
  ...
  function transport(addrs, protocol, clientOpts, listeners) {
    ...
    function retryAll() {
      _.each(rpcMap.values(), function(rpc) {
        ...
        router.getConnection().then(function (conn) {
          if (f.existy(conn))
    ...
    function installNewTopology(topology, transport) {
      ...
      return newConnected
        .then(function (newConnects) {
          ...
          emitter.emit('router', newRouter);
          // return Promise.all(newConnects.map(function (conn) {
          //   return conn.auth().then(function () {
          //     return conn;
          //   });
          // }));
    ...
    function toLazyConnectionsFirstCall(servers, connF, transport) {
      return _.map(servers, function(server) {
        return function() {
          ...
          router = new FixedRouter(logger, conn, DEFAULT_CLUSTER_NAME);
          return connF(conn);
        };
      });
    }
    ...
    function getFailoverMainRouter(failedClusterName) {
      return function(connInfo) {
        logger.debug('getFailoverMainRouter ' + failedClusterName);
    ...
    function getManualMainRouter(connInfo) {
      logger.debugf('getManualMainRouter' + connInfo);
      var clusterName = connInfo[0];
      var conn = connInfo[1];
    ...
    var o = {
      ...
      writeKeyCommand: function(ctx, key, decoder, preWrite) {
        return write(ctx, decoder, function() { return router.getConnection(key, protocol); }, preWrite);
      },
