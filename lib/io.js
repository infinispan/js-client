...
(function() {
  ...
  function transport(addrs, protocol) {
    ...
    function write(ctx, decoder, connFunc) {
      return new Promise(function (fulfill, reject) {
        var conn = connFunc();
        logger.tracef('Write buffer(msgId=%d) to %s', ctx.id, conn.toString());
        conn.write(ctx.buf);
    ...
    var o = {
      ...
      writeCommand: function(ctx, decoder) {
        return write(ctx, decoder, function() { return router.getConnection(); });
      },
