...
(function() {
  ...
  var ConsistentHashRouter = function(logger, topologyId, conns, segments, clusterName) {
    ...
    function ping(transport) {
      ...
      return transport.writeCommand(ctx, p.decodeServerMediaTypes);
    }

    function auth(transport) {
      ...
      return transport.writeCommand(ctx, p.decodeSasl);
    }
    ...
    return {
      ...
      connectAll: function() {
        var pingCall = function() {
          return ping(o).catch(function(error){
            throw error;
          });
        }
        var authCall = function() {
          return auth(o).catch(function(error){
            throw error;
          });
        }
        _.map(conns, function (conn) { return conn });
      },
  ...
  function transport(addrs, protocol, clientOpts, listeners) {
    ...
    function onRouter(r) {
      ...
      console.log('onRouter ' +  r.getConnection());
    ...
    function altConnection(clusterName) {
      ...
      return function(conn) {
        return Promise.all([Promise.resolve(clusterName), conn.connect().then(pingCall).then(authCall)]);
      }
    }
    ...
    var o = {
      connect: function() {
        ...
        var conn = findFirstConnection(lazyConns, getMainRouter);
        console.log(router);
        var pingCall = function() {
          return ping(o).catch(function(error){
            throw error;
        ...
        return conn.then(pingCall).then(authCall);
      },
      ...
      updateTopology: function(bytebuf) {
        ...
        if (topology.done && !_.isEqual(topology.id, router.getTopologyId())) {
          return installNewTopology(topology, o, this.getProtocol());
        }
