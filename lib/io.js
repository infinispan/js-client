...
(function() {
  ...
  var Connection = function(addr, transport, listeners) {
    ...
    var firstRouter =  new FixedRouter(logger, this, DEFAULT_CLUSTER_NAME);
  ...
  function transport(addrs, protocol, clientOpts, listeners) {
    ...
    function filterFirstConnection(topology, addr) {
      var currentAddrs = [addr];
    ...
    var installingNewTopology = false;
    ...
    var o = {
      ...
      updateTopology: function(bytebuf) {
        ...
        if (!installingNewTopology && topology.done && router && !_.isEqual(topology.id, router.getTopologyId())) {
          installingNewTopology = true;
