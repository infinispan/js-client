...
(function() {
  ...
  var Client = function(addrs, clientOpts) {
    ...
    function context(size) {
      return {buf: new Buffer(size), offset: 0, id: parseInt(_.uniqueId())};
    }

    function write(ctx, decoder) {
      return transport.write(ctx.id, ctx.buf, decoder);
    }
    ...
    function future(ctx, op, body, decoder, opts) {
      var topologyId = transport.getTopologyId();
      logger.tracef('Encode operation with topology id %d', topologyId);
      var encoded = encode(ctx, p.encodeHeader(op, opts, topologyId), body);
    ...
    return {
      ...
      stats: function() {
        ...
        return future(ctx, 0x15, undefined, p.decodeStringPairs);
      },
      ...
      getTopologyInfo: function() {
        return transport.getTopologyInfo();
      }
    }
  };
  ...
  exports.client = function client(args, options) {
    ...
    return c.connect().then(function() { return c; });
  };
