...
(function() {
  ...
  var Protocol = function() {
    ...
    //function decodePrevOrUndefined(status, bytebuf) {
    //  if (isSuccessWithPrev(status)) {
    ...
    return {
      encodeK: function (id, op, k, opts) {
        ...
        return chain(f.cat(header(id, op, flags), key(k, opts)));
      },
      encodeKV: function (id, op, k, v, opts) {
        var flags = encodeFlags(opts);
        return chain(f.cat(header(id, op, flags), keyValue(k, v, opts)));
      },
      ...
      decodeBody: function(header, bytebuf, opts) {
        ...
        switch (op(header)) {
          ...
          default:
            //return decodePrevOrElse(status(header), bytebuf, isSuccessWithPrev, returnUndefined);
            return undefined;
        }
      }
    };
  ...
  var Client = function(host, port) {
    ...
    function write(opts) {
      var localOpts = f.existy(opts) ? opts : {};
      return function(ctx) {
        return _connect.write(ctx.id, ctx.buf, localOpts);
      }
    }
    ...
    return {
      ...
      removeWithVersion: function(k, version, opts) {
        var localOpts = includeVersion(version, opts);
        return f.pipeline(ctx(SMALL), enc(REMOVE_WITH_VERSION, _proto.encodeK, k, localOpts), write(localOpts));
      },
      get: function(k) {
      ...
      replaceWithVersion: function(k, v, version, opts) {
        var localOpts = includeVersion(version, opts);
        return f.pipeline(ctx(MEDIUM), enc(REPLACE_WITH_VERSION, _proto.encodeKV, k, v, localOpts), write(localOpts));
      },
      clear: function () {
