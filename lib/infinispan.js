...
(function() {
  ...
  var Protocol = function() {
    ...
    var DECODE_VINT = f.actions([codec.decodeVInt()], decodedLast); // TODO: Could just return the single decoded value
    var DECODE_PAIR = f.actions([codec.decodeObject(), codec.decodeObject()], decodedValues);

    function header(id, op, opts) {
    ...
    function multiKeyValue(pairs, opts) {
      var base = [
        codec.encodeUByte(0x88),      // infinite lifespan & max idle
    ...
    return {
      encodeK: function (id, op, k, opts) {
        return chain(f.cat(header(id, op, opts), key(k, opts)));
      },
      encodeKV: function (id, op, k, v, opts) {
        return chain(f.cat(header(id, op, opts), keyValue(k, v, opts)));
      },
      encodeMultiKV: function (id, op, pairs, opts) {
        return chain(f.cat(header(id, op, opts), multiKeyValue(pairs, opts)));
      },
      encodeMultiK: function (id, op, keys, opts) {
        return chain(f.cat(header(id, op, opts), multiKeys(keys, opts)));
      },
      encode: function (id, op) {
        return chain(header(id, op));
      },
  ...
  var Client = function(host, port) {
    ...
    function write(opts) {
      return function(ctx) {
        return _connect.write(ctx.id, ctx.buf, opts);
      }
    }

    function includeVersion(ver, opts) {
      if (f.existy(opts)) {
        opts.version = ver;
        return opts;
      }
    ...
    return {
      ...
      put: function(k, v, opts) {
        return f.pipeline(ctx(MEDIUM), enc(PUT, _proto.encodeKV, k, v, opts), write(opts));
      },
      ...
      remove: function(k, opts) {
        return f.pipeline(ctx(SMALL), enc(REMOVE, _proto.encodeK, k, opts), write(opts));
      },
      ...
      putIfAbsent: function(k, v, opts) {
        return f.pipeline(ctx(MEDIUM), enc(PUT_IF_ABSENT, _proto.encodeKV, k, v, opts), write(opts));
      },
      ...
      replace: function(k, v, opts) {
        return f.pipeline(ctx(MEDIUM), enc(REPLACE, _proto.encodeKV, k, v, opts), write(opts));
      },
      ...
      replaceWithVersion: function(k, v, version, opts) {
        var optsWithVersion = includeVersion(version, opts);
        return f.pipeline(ctx(MEDIUM), enc(REPLACE_WITH_VERSION, _proto.encodeKV, k, v, optsWithVersion), write(opts));
      },
      ...
      removeWithVersion: function(k, version, opts) {
        var optsWithVersion = includeVersion(version, opts);
        return f.pipeline(ctx(SMALL), enc(REMOVE_WITH_VERSION, _proto.encodeK, k, optsWithVersion), write(opts));
      },
      // Returns an array of {key: <K>, value: <V>} pairs
      // TODO: Validate empty
      getAll: function(keys, opts) {
        return f.pipeline(ctx(MEDIUM), enc(GET_ALL, _proto.encodeMultiK, keys, opts), write());
      },
      ...
      putAll: function(pairs, opts) {
        return f.pipeline(ctx(BIG), enc(PUT_ALL, _proto.encodeMultiKV, pairs, opts), write());
      },
      clear: function () {
        return f.pipeline(ctx(TINY), enc(CLEAR, _proto.encode), write());
      },
      ping: function () {
        return f.pipeline(ctx(TINY), enc(PING, _proto.encode), write());
      }
    }
  };
