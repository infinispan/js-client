      ...
      connect: function(client, host, port) {
        return new Promise(function (succeed, fail) {
          ...
          _client.on('data', function(data) {
            var decodeHeaderChain = f.actions(p.decodeHeader(), decodedValues);
  ...
  var Client = function(host, port) {
    ...
    return {
      ...
      replaceWithVersion: function(k, v, version, opts) {
        var decoder = p.decodePrevOrElse(opts, p.hasSuccess, p.hasSuccess);
        return future(MEDIUM, 0x09, p.encodeKeyValueVersion(k, v, version), decoder, opts);
      },
      ...
      putAll: function(pairs, opts) {
        return future(BIG, 0x2D, p.encodeMultiKeyValue(pairs), _.constant(undefined), opts);
      },
