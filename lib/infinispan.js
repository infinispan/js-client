...
(function() {
  ...
  var PUT = 0x01, GET = 0x03
    ...
    , REPLACE_VERSIONED = 0x09, REMOVE = 0x0B
  ...
  var Client = function(host, port) {
    ...
    return {
      ...
      put: function(k, v) {
        return write(mkBuf(MEDIUM), encode(PUT, k, v));
      },
      remove: function(k) {
        return write(mkBuf(SMALL), encode(REMOVE, k));
      },
      get: function(k) {
        return write(mkBuf(SMALL), encode(GET, k));
      },
      putIfAbsent: function(k, v) {
        return write(mkBuf(MEDIUM), encode(PUT_IF_ABSENT, k, v));
      },
      replace: function(k, v) {
        return write(mkBuf(MEDIUM), encode(REPLACE, k, v));
      },
      clear: function () {
        return write(mkBuf(TINY), encode(CLEAR));
      },
      getVersioned: function(k) {
        return write(mkBuf(SMALL), encode(GET_VERSIONED, k));
      }
    }
  };
