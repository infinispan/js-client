...
(function() {
  ...
  var Client = function(addrs, clientOpts) {
    ...
    return {
       ...
       * If the batchSize is set to 0, no entries will be transferred from the server. The returned promise wouldn't
       * contain the key/value pairs, but will contain {done:true} key/values as much as the elements in the cache.
  ...
  var TopologyInfo = function(transport) {
    return {
       ...
       * If the clusterName passed is {@code null} then the remote cache
       * manager will be switched to default cluster and will return {code true}.
       * If the clusterName points to non-existent
       * cluster, the switch won't take place and will return {code false}.
       * @memberof Topology#
       * @since 0.4
       */
