...
(function() {
  ...
  var Protocol = function() {
    ...
    function completeWithExecuted(header) {
      console.log("Complete with exist");
      var status = extractStatus(header);
      if (status == SUCCESS)
        return true;
    ...
    return {
      encode: function (msgId, opCode, k, v) {
        var encodeFun = f.existy(v) ? encodeKeyValue(k, v) : encodeKey(k);
          return f.actions(
            f.cat(encodeHeader(msgId, opCode), encodeFun),
            totalBytes);
      },
      //encodeKeyValueOp: function (msgId, opCode, k, v) {
      //  return f.actions(
      //    f.cat(encodeHeader(msgId, opCode), encodeKeyValue(k, v)),
      //    totalBytes);
      //},
      //encodeKeyOp: function (msgId, opCode, k) {
      //  return f.actions(
      //    f.cat(encodeHeader(msgId, opCode), encodeKey(k)),
      //    totalBytes);
      //},
      ...
      decodeBody: function(header, bytebuf) {
        ...
        switch (extractOpCode(header)) {
          ...
          case PUT_IF_ABSENT_OUT: return completeWithExecuted(header);
  ...
  var Client = function(host, port) {
    ...
    function withBuffer(size, fun) {
      var bytebuf = {buf: new Buffer(size), offset: 0};
      fun(bytebuf);
      return bytebuf;
    }

    function small(fun) {
      // TODO: Change size to 32 once resizing has been implemented
      return withBuffer(128, fun);
    }

    function medium(fun) {
      // TODO: Change size to 64 once resizing has been implemented
      return withBuffer(128, fun);
    }

    function write(fun) {
      var msgId = _msgIdCounter.incr();
      //var bytebuf = mediumBuffer(_proto.encodeKeyValueOp(msgId, PUT_IN, k, v));
      var bytebuf = fun(msgId);
    ...
    return {
      ...
      put: function(k, v) {
        return write(function (id) { return medium(_proto.encode(id, PUT_IN, k, v)) } );
        //var msgId = _msgIdCounter.incr();
        //var bytebuf = mediumBuffer(_proto.encodeKeyValueOp(msgId, PUT_IN, k, v));
        //return _connect.write(msgId, bytebuf.buf);
      },
      remove: function(k) {
        return write(function (id) { return small(_proto.encode(id, REMOVE_IN, k)) } );
        //var msgId = _msgIdCounter.incr();
        //var bytebuf = smallBuffer(_proto.encodeKeyOp(msgId, REMOVE_IN, k));
        //return _connect.write(msgId, bytebuf.buf);
      },
      get: function(k) {
        return write(function (id) { return small(_proto.encode(id, GET_IN, k)) } );
        //var msgId = _msgIdCounter.incr();
        //var bytebuf = smallBuffer(_proto.encodeKeyOp(msgId, GET_IN, k));
        //return _connect.write(msgId, bytebuf.buf);
      },
      putIfAbsent: function(k, v) {
        return write(function (id) { return medium(_proto.encode(id, PUT_IF_ABSENT_IN, k, v)) } );
        //var msgId = _msgIdCounter.incr();
        //var bytebuf = mediumBuffer(_proto.encodeKeyValueOp(msgId, PUT_IN, k, v));
        //return _connect.write(msgId, bytebuf.buf);
      },
