...
(function() {
  ...
  var Client = function(addrs, clientOpts) {
    ...
    return {
        ...
        return transport.connect()
            .then(function() {
              if(transport.authOpts().enabled) {
                // if authentication enabled, authenticate
      ...
      //       // .then(function() {
      //       //   if(transport.authOpts().enabled) {
      //       //     logger.debugf('Authenticate to Infinispan %s', client);
      //       //     return client.authenticate();
      //       //   }
      //       //   return client;
      //       // })
       ...
       *
       * @returns {module:promise.Promise}
       * A promise that will be completed when auth mechanisms response was received.
       * @memberof Client#
       * @since 0.8
       */
      authMechList: function() {
        var ctx = transport.context(TINY);
        logger.debugf('Invoke auth(msgId=%d)', ctx.id);
        return futureDecodeOnly(ctx, 0x21, p.decodeAuthMech);
      },
       ...
       *
       * @returns {module:promise.Promise}
       * A promise that will be completed when the client is authenticated.
       * @memberof Client#
       * @since 0.8
       */
      authenticate: function() {
        var ctx = transport.context(TINY);
        logger.debugf('Invoke auth(msgId=%d)', ctx.id);
        return future(ctx, 0x23, p.sasl(transport.authOpts()), p.decodeSasl);
      },
